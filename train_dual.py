...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Trainloader
    train_annot_path = os.path.join(opt.annot_dir, 'train_bboxes_norm.json')
    train_loader, dataset = create_dataloader(train_path,
                                              ...
                                              labels_json=train_annot_path)
    labels = np.concatenate(dataset.labels, 0)
    mlc = int(labels[:, 0].max())  # max label class
    assert mlc < nc, f'Label class {mlc} exceeds nc={nc} in {data}. Possible class labels are 0-{nc - 1}'

    # Process 0
    val_annot_path = os.path.join(opt.annot_dir, 'val_bboxes_norm.json')
    if RANK in {-1, 0}:
...
def parse_opt(known=False):
    ...
    # Custom
    # add argument for processing_scripts directory
    parser.add_argument('--annot_dir', type=str, default='./processing_scripts', help='path to processing scripts')
